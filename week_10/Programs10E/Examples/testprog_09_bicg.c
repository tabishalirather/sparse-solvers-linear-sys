...
int main() {
  ...
  /* Problem from the matrix market collection. */
  A = csr_import_from_mm_mtx("../../TestMatrixCollection/orsirr_1.mtx");
  assert(A);
  assert(A->nr == A->nc);
  n = A->nr;
  printf("Matrix ORSIRR1:\n");
  printf("  Dimension = %d\n", n);
  printf("  Non-zeros = %d\n", A->nz);

  /* All one exact solution and corresponding right hand side. */
  ...
  /* Counter for the current iterative scheme. */
  k = 0;
  ...
  errvec[k] = malloc((maxit[k] + 1) * sizeof(double));
  vec_copy(n, x0, x);
  csr_bicg(A, b, x, tol, stopcrit, maxit[k], &esterr, &iter[k], errvec[k]);
  ...
  errvec[k] = malloc((maxit[k] + 1) * sizeof(double));
  vec_copy(n, x0, x);
  csr_cgs(A, b, x, tol, stopcrit, maxit[k], &esterr, &iter[k], errvec[k]);
  ...
  errvec[k] = malloc((maxit[k] + 1) * sizeof(double));
  vec_copy(n, x0, x);
  csr_bicgstab(A, b, x, tol, stopcrit, maxit[k], &esterr, &iter[k], errvec[k]);
