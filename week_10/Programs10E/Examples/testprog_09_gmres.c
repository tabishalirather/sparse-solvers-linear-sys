...
int main(int argc, char **argv)
{
  ...
  int restart = 8;
  for (; restart >= 0; restart -= 1) {
    printf("GMRes(%d)\n", restart);
    /* GMRes with a default start vector x0 = NULL. */
    gmres_t *gmres = gmres_new(A, b, NULL);
    gmres_iter_max(gmres, iter_max);
    ...
    /* Get the number of iterations that were carried out. */
    int iter = gmres_iter(gmres);
    printf("  iter = %d\n", iter);

    /* Add line to plot. */
    conv_plot_line(conv_plot, iter, gmres_residual_norms(gmres));
    conv_plot_namef(conv_plot, "GMRes(%d)", restart);
  ...
  conv_plot_save(conv_plot, "testprog_08_gmres");
