...
void cg_rhs(int k, int low) {
  ...
  /* Anzahl der in der rechten Seite enthaltenen Eigenvektoren. */
  eignum = 1;
  for (i = 0; i < d; ++i) {
    eignum *= (k + 1);
  }
  ...
  if (low != 1) {
    ...
    while (1) {
      ...
      /*                                      */
      /*          TODO --- Problem 2          */
      /* Compute the eigenvector for the      */
      /* multi-index eigidx and add it the    */
      /* right-hand side.                     */
      /*                                      */
      /* ------------------------------------ */

      /* Compute the eigenvector corresponding to eigidx. */
      laplace_ev(n,d,eigidx,eigvec); //cg_rhs fxn is supposed to call CG method with zero vector as staritng egctor and a linear combination of J number of eigen vectors as RHS, it will compute the linear combination of J number of lower or highest eigen values calls CG on Ax = j linear combination of vectors, where J = (k+1)^d
      /* Add the eigenvector to the right-hand side. */
    ...
    while (1) {
      ...
      /* Proceed to the next multiindex. */
      for (i = 0; i < d && eigidx[i] == eigidxmax; ++i) {
        eigidx[i] = eigidxmin;
      }
...
int main() {
  ...
  convplot_steps(
    "testprog_06_cg_rhs_low.eps",
    "CG method with different right hand sides, lowest eigenvalues",
    "Iteration steps",
    "Residual norm",
    NPLOTS,
    iter,
