...
void csr_test_ordering(csr *A, char *name, int start)
{
  ...
  snprintf(filename, slen, "%s_before.eps", name);
  ...
  for (level_set_variant = 0; level_set_variant < 3; ++level_set_variant) {
    ...
    if (!perm_check(perm)) {
      printf("Error: index vectors are not representing a permutation and its inverse.\n");
    }
    ...
    snprintf(filename, 100, "%s_after%s.eps", name,
             level_set_string[level_set_variant]);
