...
int *perminv(int n, const int *p) {
  printf("--------------------Start perminv------------------------------------\n");
  int i, *pinv;
  printf("calling pinv from Library/perm.c \n");
  pinv = (int *) malloc(n * sizeof(int));
  printf("Intialiazed pinv is:"); print_vec(n, pinv);
  // refer to perm-not-efficient to see the matrix implementation of this method.
...
{
  printf("\n--------------------Start vec_permute------------------------------------\n");
  // printf("This is a print from vec_permute in perm.c\n");
  int i;
  double *Pb;
  int *p;
  double *Pinvb;
  Pb = (double *) malloc(n*sizeof(double));
  Pinvb = (double *) malloc(n*sizeof(double));
  p = (int *) malloc(n*sizeof(double));
  // printf("before make p: ");print_vec(n, p);
  printf("\n");
  make_pinv(n,p,pinv);
  // printf("\n");
  // printf("after make pinv: ");print_vec(n, p);
  ...
  //permute with p:
  /*p = [1 3 2 0]
  send idx=0 element of b to idx=1 in Pb
  send idx=1 element of b to idx=3 in Pb
  send idx=2 element of b to idx=2 in Pb
  send idx=3 element of b to idx=0 in Pb
  b = [1 4 3 2]*/
  for (int i=0; i<n; i++) {
    Pb[p[i]] = b[i];
  }
  printf("Pinvb: ");print_vec_db(n, Pinvb);
  printf("\n");
  printf("Pb:    ");print_vec_db(n, Pb);
