...
csr *csr_alloc(int rows, int cols, int elements)
{
  ...
  new->row     = (int *)  malloc((rows+1)*sizeof(int)); // pointer to start of each row
  new->col     = (int *)  malloc(elements*sizeof(int)); // col-idx to the start of each column
  new->coeff = (double *) malloc(elements*sizeof(double)); // non zero values in the matrix.
