...
int main(void)
{
  ...
  /* Counter for different methods. */
  int j = 0;

  /* Number of methods to be tested. */
  int k = 6;

  /* Parameters for test matrix. */
  int n = 10;
  int d = 3;
  ...
  A  = csr_laplace(n + 1, d);
  b  = (double *) malloc(A->nc*sizeof(double));
  x  = (double *) malloc(A->nc*sizeof(double));
  ...
  m[j] = maxit;
  vec_scale(A->nc, 0.0, x);
  csr_steepest_descent(A, b, x, tol, ITSOLVER_RELRES, m[j],
    &esterr, &iter, vecvec[j]);
  m[j] = iter;
  printf("\n");
  ...
  m[j] = maxit;
  vec_scale(A->nc, 0.0, x);
  csr_minimal_residual(A, b, x, tol, ITSOLVER_RELRES, m[j],
    &esterr, &iter, vecvec[j]);
  m[j] = iter;
  printf("\n");
  ...
  m[j] = maxit;
  vec_scale(A->nc, 0.0, x);
  csr_residual_norm_steepest_descent(A, b, x, tol, ITSOLVER_RELRES, m[j],
    &esterr, &iter, vecvec[j]);
  m[j] = iter;
  printf("\n");
